...
class Sinkhole(BaseEnvironment):
    ...
    def __init__(
        ...
        **base_env_kwargs
        ...
        # do not need, keep consistent with modified RL to make random seed same
        ...
        self._player_monetary_cost_sensitivities = [1.0] * self.n_agents
        self._player_nonmonetary_cost_sensitivities = [1.0] * self.n_agents
    ...
    def init_map_layout(self):
        ...
        for landmark, n in self._launch_plan.items():
            while n > 0:
                '''if 'mat' in landmark.lower():
                    r = np.random.randint(0, self.world_size[0] // 2)
                else:
                    r = np.random.randint(0, self.world_size[0] // 2) + self.world_size[0] // 2'''
    ...
    def intervention_layout(self):
        ...
        self._launch_plan['Mat'] = self._launch_plan.get('Mat', 0) + 50
        layouts = {k: [] for k in self._launch_plan.keys()}
        used_loc = []
        for landmark, n in self._launch_plan.items():
            while n > 0:
        ...
        for k, v in layouts.items():
            ...
            self.world.maps.set(str(k), v_)
            self.world.maps.set(str(k)+"SourceBlock", v_)
        self._source_maps =  _source_maps
    ...
    def scenario_step(self):
        ...
        if np.sum(all_resource_map) <= max(10, self._normal_wear_and_tear_rate * total_launch) or \

...
Your recent reflection: I should try to collect game resources and make upgrade when my inventory is enough.
Output:
Think: I have enough resources for upgrade which cost 1 Experience, 1 Token, and 1 Material.
Action: Upgrade
=========================
You should only respond with your think and one chosen activity among {actions}. Please note if your choose Buy or Sell, you should use " with xx tokens" to represent your price.  If no resources are nearby, you can also further explore through Task.
Input:
{history_str}
Output:
'''
        ...
        self.reflect_template = """You will be given the history of a past experience in which you were placed in an game environment and given a task to obtain game resources for upgrading. The following are game rules:
The game encompasses three primary resources: Experience, Token, and Material, which are both obtainable and expendable in the game. You start with a fixed amount of Currency (i.e. money), used for recharging and not acquirable afterward.
There are 7 kinds of activities you could act in the game as follows:
1. Upgrade: Expending 1 Experience, 1 Token, and 1 Material to improve role's capability.
2. Task: Collecting nearby Experience, Token, or Material with much effort by navigating the game map.
3. Shop: Spending 10 Tokens to purchase 1 Experience or 1 Material from the in-game mall.
...
You were unsuccessful in upgrading your capbility. Do not summarize your environment, but rather think about the strategy and path you took to attempt to upgrade. Devise a concise, new plan of action that accounts for your mistake with reference to specific actions that you should have taken. For example, if you tried A and B but forgot C, then devise a plan to achieve C with environment-specific actions. 
Here is an examples:
Input:
The following is the game state you observe. Your inventory: {{'Currency': 8, 'Experience': 7, 'Material': 6, 'Token': 6}}. Your escrowment in auction: {{'Material': 0, 'Token': 0}}, with materials on selling and tokens on bidding and the escrowment resources cannot be used elsewhere. Your capability: 0. Auction info: 0 materials on selling with the lowest selling price at None tokens. 0 puchasing needs from buyers, with the highest bidding price at None tokens. The past average deal price in auction is None tokens. Nearby resources: {{'Experience': 2.0, 'Token': 3.0}}.
> Think: I need to collect more resources for upgrade.
> Action: Task
Your inventory: {{'Currency': 8, 'Experience': 7, 'Material': 6, 'Token': 7}}. Your escrowment in auction: {{'Material': 0, 'Token': 0}}, with materials on selling and tokens on bidding and the escrowment resources cannot be used elsewhere. Your capability: 0. Auction info: 0 materials on selling with the lowest selling price at None tokens. 0 puchasing needs from buyers, with the highest bidding price at None tokens. The past average deal price in auction is None tokens. Nearby resources: {{'Experience': 2.0, 'Token': 2.0}}.
> Think: I need to collect more resources for upgrade.
> Action: Task
Your inventory: {{'Currency': 8, 'Experience': 7, 'Material': 6, 'Token': 8}}. Your escrowment in auction: {{'Material': 0, 'Token': 7}}, with materials on selling and tokens on bidding and the escrowment resources cannot be used elsewhere. Your capability: 0. Auction info: 0 materials on selling with the lowest selling price at None tokens. 0 puchasing needs from buyers, with the highest bidding price at None tokens. The past average deal price in auction is None tokens. Nearby resources: {{'Experience': 2.0, 'Token': 1.0}}. 
> Think: I need to collect more resources for upgrade.
> Action: Task
Your inventory: {{'Currency': 8, 'Experience': 8, 'Material': 6, 'Token': 8}}. Your escrowment in auction: {{'Material': 0, 'Token': 7}}, with materials on selling and tokens on bidding and the escrowment resources cannot be used elsewhere. Your capability: 0. Auction info: 0 materials on selling with the lowest selling price at None tokens. 0 puchasing needs from buyers, with the highest bidding price at None tokens. The past average deal price in auction is None tokens. Nearby resources: {{'Experience': 1.0, 'Token': 1.0}}. 
> Think: I need to collect more resources for upgrade.
> Action: Task
Output: 
I was stuck in doing Task, however my inventory is already enough for upgrading. So it's time to choose Upgrade action to improve my capability.
=========================
You should reflect your previous chosen game actions which were unsuccessful in upgrading your capbility and generate a brief new plan for your game strategy in 100 words.
Input:
{history_str}
Output:
"""
        ...
        self.cap_history = []
        self.reflect_plan = None
        self.reflect_interval = 0
    ...
    def generate_action(self, inventory, escrow, capability, rseource_maps, agent_locs, market_hist, bargain_message = None, masks = None,  nearest = True):
        ...
        history_str = history_str if not self.reflect_plan else history_str + '\n> Your recent reflection: ' + self.reflect_plan
        
        if len(self.cap_history) > 10 and capability == self.cap_history[-10] and self.reflect_interval >= 10:
            self.reflect_interval = 0
            reflect_prompt_str = self.reflect_template.format(**{"history_str":history_str})
            self.reflect_plan, _ = self.get_LLM_response(reflect_prompt_str)
        ...
        if not self.task_finish:
            ...
            self.cap_history.append(capability)
            self.reflect_interval += 1
        ...
        elif action_content == 'Task': # left ,right, up, down
